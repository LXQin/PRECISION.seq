...
We include functions for two methods, edgeR and voom-limma, to assess differential expression. The input can be either the normalized dataset (from re-scaling normalization) or the un-normalized dataset with adjustment factor (from regression-based normalization). The output includes the p-values for the miRNAs/markers, and their group mean differences (log2 fold changes), and the names of the significantly differentially expressed miRNAs/markers (using a p-value cutoff specified by the user).
```{r}
...
We connect the depth-normalization step and the differential-expression-analysis step with functions that combine the functions for these two steps.
```{r}
