...
We normalize each simulated test data set using `vsn` normaliztion.
We then use the pipeline function [`pip.simulated.data`](../reference/pip.simulated.data.html) to compute the agreement of differential expression statuses for each simulated data set.
The results are summarized in a boxplot diagram, which is returned by the pipeline function.
