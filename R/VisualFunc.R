...
#'
#' @examples
#' plot.RLE(test, "test without normalization")
plot.RLE = function(normtest, main) {
...
#' @examples
#' t = runif(1033)
#' names(t) = rownames(test)
#' plot.CAT("Scale", "Example", t)
#'
plot.CAT = function(MethodType, MethodName, pvalues){
  benchmark.p = DE.voom(benchmark, c(rep(c(rep('MXF',9),rep('PMFH',9)),3)))$p.val[,1]
  if(MethodType == "Scale"){
